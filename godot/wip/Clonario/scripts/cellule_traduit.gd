...
@export var energie_chasse_max := 5.0  # Secondes de chasse avant fatigué
@export var energie_fuite_max := 3.0   # Secondes de fuite avant fatigué
...
func def_vie(nouvelle_vie : float) -> void:
	vie = nouvelle_vie
	...
	def_taille(sprite.texture.get_size().x * scale.x)
...
func _physics_process(delta: float) -> void:
	...
	if controleur != null:
		...
		var comportement = controleur.obtenir_comportement()  # "chasse", "fuite", ou "repos"
		var peut_chasser = energie_chasse > 0
		var peut_fuir = energie_fuite > 0
		
		var vitesse_cible = controleur.obtenir_mouvement_avec_energie(peut_chasser, peut_fuir)
...
func surveiller_chevauchement() -> void:
	...
	for obj in zone_activation.get_overlapping_areas():
		...
		if obj is Area2D and obj.get_parent() is Cellule:
			...
			if autre_cellule.obtenir_taille() == taille:
			...
			if (distance < obtenir_taille()/3):
				print("Distance: " + str(distance) + "\t Taille : " + str(autre_cellule.obtenir_taille()))
			
				if autre_cellule.obtenir_taille() < taille:
					print("Distance: " + str(distance) + "\t Taille : " + str(autre_cellule.obtenir_taille()))
					autre_cellule.mourir()
					grandir(autre_cellule.obtenir_vie() * 0.25)
					continue
				else:
					mourir()
					continue
...
func def_controleur(le_controleur : Controleur) -> void:
	controleur = le_controleur
	add_child(controleur)
	message_pas_controleur = false

func obtenir_sprite() -> Sprite2D:
	return sprite

func obtenir_vie() -> float:
	return vie

func obtenir_rayon() -> float:
	return rayon

func obtenir_taille() -> float:
	return taille

func def_taille(nouvelle_taille : float) -> void:
	taille = nouvelle_taille

func def_etiquette(nouveau_nom : String) -> void:
	name = nouveau_nom
	$NameLabel.text = nouveau_nom

func def_tous_objets_mangeables(objets : Array) -> void:
	var tous_objets_mangeables := []
	tous_objets_mangeables = objets
	controleur.def_objets_mangeables(tous_objets_mangeables)
